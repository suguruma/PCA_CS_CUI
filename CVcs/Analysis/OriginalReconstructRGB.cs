...
namespace CVcs.Analysis
{
    class OriginalReconstructRGB
    {
        ...
        public OriginalReconstructRGB(string inputstr)
        {
            // フォルダ作成のため
            ...
            using (CvMat pjt_matT = Cv.CreateMat(numImage, inline, MatrixType.F32C1))
            {
                ...
                //----- 係数の読み込み -----//
                Console.Write("Confference Mat Read ... ");
                filename = savefolder + inputstr;
                ReadMat(pjt_matT, filename);

                pjt_mat = Cv.CreateMat(inline, numImage, MatrixType.F32C1);
                Cv.Transpose(pjt_matT, pjt_mat);
                Console.Write("OK\n");
        ...
        public void ReadMat(CvMat input_mat, string str)
        {
            ...
            using (StreamReader rf = new StreamReader(str))
            {
                string line;
                string[] parts;
                string[] separator = { ", ", Environment.NewLine }; // セパレート文字列(優先順位:左から)
                int x, y;
